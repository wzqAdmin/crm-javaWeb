...
<script type="text/javascript">
	...
	$(function () {
		...
		$("#searchActivityBtn").keydown(function (event) {
			...
			if(event.keyCode==13){
				$.ajax({
					...
					url:"workbench/clue/getActivityListByActivityNameAndClueId.do",
					type:"get",
					data:{
					  "clueId":"${c.id}",
					  "aname":$.trim($("#searchActivityBtn").val())
					},
					dataType:"json",
					success:function (data) {
						...
						$.each(data,function (i,n) {
						    ...
						    html+='<tr>';
						    /*
						      此时的n.id是activity的id，因为我们后期需要添加关联，实际上就是在市场活动和线索联系表上添加一条记录
						      需要的条件由，市场活动的id，当前线索的id

						      而设置name属性的原因是，我们还支持一次关联多条记录
						     */
							html+='<td><input type="checkbox" name="xz" class="fx" value="'+n.id+'"/></td>';
							html+='<td>'+n.name+'</td>';
				...
				//在模态窗口内，当点击了回车，这个事件执行完毕后会清空原有页面的内容，此时我们要强制结束方法执行，防止清空
				return false;
			}
		})
		/*----------------------------------------------------------------------------------*/
		...
		$("#bundBtn").click(function () {
			...
			//获取用户选择的复选框，因为我们支持复选关联
             ...
             if($xz.length==0){
				 ...
				 //参数=/workbench/clue/bund.do?cId=xxx&aId=xxx&aId=xxx
				 var param="cId=${c.id}&";
		...
		//当所有的复选框都被选中的时候，全选框也要被选择，所以要给所有的复选框绑定事件
		/*
	...
	function unbund(id) {
	    ...
	    if(window.confirm("你确定要解除这条关联关系吗？不可回退！")){
            $.ajax({
                url:"workbench/clue/unbund.do",
                type:"post",
                data:{
                   "id":id   //注意：这个id是关联关系表的id
                },
                dataType:"json",
                success:function (data) {
                    /*
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button"  id="bundBtn" class="btn btn-primary" <%--data-dismiss="modal"--%>>关联</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		<div class="page-header">
			<h3>${requestScope.c.fullname}${c.appellation} <small>${c.company}</small></h3>
		</div>
